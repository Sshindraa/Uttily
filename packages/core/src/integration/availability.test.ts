...
describe.skipIf(!isCi && !process.env.DATABASE_URL)(
  'Availability integration — InventoryBlock',
  () => {
    // -------------------------------------------------------------------------
    // 1. CRUD basique
    // -------------------------------------------------------------------------
    ...
    it('transition invalide : libérer un bloc CONVERTED rejeté', async () => {
      ...
      await expect(releaseBlock(db, organizationId, hold.id)).rejects.toThrow(
        /Transition invalide/,
      );
    });

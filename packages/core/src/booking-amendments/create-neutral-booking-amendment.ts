 ...
 *
 * Mutation transactionnelle idempotente qui crée et applique atomiquement un
 * amendement NEUTRAL (delta financier nul) sur une réservation CONFIRMED.
 *
 * Reprice les amendements avec quoteFlexiblePricing (plans ACTIVE autoritatifs, G7P).
 *
 * Append-only : aucune mutation de bookings, booking_lines, booking_items ou
 * des snapshots originaux. L'amendement est inséré en READY_TO_APPLY puis
 * transitionné vers APPLIED dans la même transaction. Les blocs d'inventaire
 * sont libérés/créés selon le diff d'allocations.
 *
 * Tenant-safe : une réservation d'une autre organisation produit NOT_FOUND.
 *
 * Concurrence : lockOrganization → lockKey → bookings FOR UPDATE →
 * booking_amendments FOR UPDATE → inventory_blocks FOR UPDATE (ORDER BY id).
 * L'optimistic locking via expectedLastAppliedAmendmentNumber garantit qu'un
 * deuxième appel concurrent basé sur le même état effectif reçoit STALE.
 */

import { createHash, randomUUID } from 'node:crypto';
import { and, asc, eq, inArray, isNull, not, sql, exists } from 'drizzle-orm';
...
import { NeutralAmendmentError } from './types-amendment';
...
const AMENDMENT_NEUTRAL_OPERATION = 'booking-amendment-neutral';

/**
 * Erreur interne typée pour casser un savepoint et propager un résultat métier.
 * Capturée dans le catch externe de la transaction.
 */
class BusinessSignal {
  readonly result: NeutralAmendmentResult;
  constructor(result: NeutralAmendmentResult) {
    this.result = result;
  }
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Validation (pure, avant tout accès DB)
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Valide la commande avant tout accès base de données.
...
// ─────────────────────────────────────────────────────────────────────────────
// Empreinte canonique (SHA-256)
// ─────────────────────────────────────────────────────────────────────────────

export function computeAmendmentFingerprint(command: NeutralAmendmentCommand): string {
...
/**
 * Calcule le diff entre les lignes effectives courantes et les lignes désirées.
 *
 * Règle de matching stricte :
 * - logicalLineId fourni :
 *   - Si introuvable dans effectiveLines → INVALID_INPUT.
 *   - Si variantId ne correspond pas à la ligne effectives → INVALID_INPUT.
 *   - Si match → MODIFY (si qty changée) ou UNCHANGED. originType et sourceBookingLineId conservés.
 * - logicalLineId non fourni (nouvelle ligne) :
 *   - Si variantId est déjà présent dans l'état effectif → INVALID_INPUT (le caller doit fournir le logicalLineId).
 *   - Sinon → ADD avec un nouveau logicalLineId (originType = AMENDMENT, sourceBookingLineId = null).
 * - Lignes effectives non référencées dans desiredLines → REMOVE.
 */
...
function extractReplayResult(record: {
  ...
  if (
    ...
    return { kind } as NeutralAmendmentResult;
  }
  if (
  ...
  if (
    ...
    (obj['classification'] === 'REFUND' || obj['classification'] === 'SUPPLEMENT') &&
    typeof obj['deltaMinor'] === 'number'
  ) {
    return {
      kind: 'FINANCIAL_ACTION_REQUIRED',
      classification: obj['classification'] as 'REFUND' | 'SUPPLEMENT',
...
export async function createNeutralBookingAmendment(
  ...
  // 1. Validation de l'acteur et de la commande (AVANT reserveKey, aucune mutation DB)
  ...
  // 2. Autorisation (avant idempotency, avec DatabaseClient)
  let membership;
  try {
  ...
  const fingerprint = computeAmendmentFingerprint(command);

  // 4. reserveKey
  const reservation = await reserveKey(db, {
    organizationId,
    operation: AMENDMENT_NEUTRAL_OPERATION,
    key: command.idempotencyKey,
    requestFingerprint: fingerprint,
  });

  if (reservation.kind === 'REPLAY') {
    const replay = extractReplayResult(reservation.record);
  ...
  // 5. Transaction principale
  return await db.transaction(async (tx) => {
    await lockOrganization(tx, organizationId);

    const lock = await lockKey(tx, reservation.record.id);
    if (lock.kind === 'REPLAY') {
      const replay = extractReplayResult(lock.record);
      if (replay) return replay;
      return { kind: 'IDEMPOTENCY_CONFLICT' } as NeutralAmendmentResult;
    }

    // LOCKED : re-vérifier l'autorisation sous lockOrganization (TOCTOU protection)
    {
      ...
      if (txMembershipRows.length === 0 || txMembershipRows[0]!.status !== 'ACTIVE') {
        ...
        return { kind: 'FORBIDDEN' } as NeutralAmendmentResult;
      }
    ...
    let businessResult: NeutralAmendmentResult;
    try {
      ...
      if (isExclusionViolation(error, 'no_overlapping_blocks')) {
        const conflictResult: NeutralAmendmentResult = {
    ...
    if (businessResult.kind === 'SUCCESS') {
      await completeKey(tx, reservation.record.id, {
        ...
        responseBody: {
          kind: 'SUCCESS',
          amendmentId: businessResult.amendmentId,
          amendmentNumber: businessResult.amendmentNumber,
        },
      });
    } else {
      await failKey(tx, reservation.record.id, {
...
// ─────────────────────────────────────────────────────────────────────────────
// Logique métier (dans le savepoint)
// ─────────────────────────────────────────────────────────────────────────────
...
async function executeBusinessLogic(
  ...
  // 0. Locks ADR-023 : bookings FOR UPDATE → booking_amendments FOR UPDATE
  ...
  // 5. Charger la location (buffers, timezone)
  ...
  if (locRows.length === 0) {
    throw new NeutralAmendmentError(
      'INTERNAL',
  ...
  // 6. Appeler quoteFlexiblePricing avec plans ACTIVE autoritatifs (dans la transaction sp)
  let quoteResult;
  try {
  ...
  // 7. Dériver customer_start_at / customer_end_at
  ...
  // 9. Valider le catalogue pour chaque variante
  ...
  // Remplir les prix et snapshots after dans le line diff à partir du quoteResult
  ...
  // 13. Allocations : calculer le diff
  ...
  // 14. Verrouiller tous les inventory_blocks affectés (ORDER BY id)
  ...
  // 15. INSERT booking_amendments (status=READY_TO_APPLY)
  ...
  // 16. INSERT booking_amendment_lines avec snapshots G7P complets
  ...
  // Mettre à jour les amendmentLineId du plan d'allocations avec les IDs réels
  for (const alloc of allocationPlan.allocations) {
  ...
  // 17. INSERT booking_amendment_allocations (status=PROPOSED)
  ...
  // 18. Mutations de blocks
  // a. RELEASE old blocks pour REMOVE et REPLACE
  for (const alloc of allocationPlan.allocations) {
  ...
  // b. INSERT nouveaux blocks pour ADD et REPLACE
  ...
  // 19. UPDATE allocations : PROPOSED → CONVERTED ou RELEASED (SANS modifier la colonne action)
  for (const alloc of allocationPlan.allocations) {
...
async function computeAllocationPlan(
  ...
  for (const [variantId, desiredQty] of desiredQtyByVariant) {
    ...
    for (let i = 0; i < retainedCount; i++) {
      ...
      const sourceBlockId = await findSourceBlockId(sp, organizationId, bookingId, ea);
      const datesChanged =
    ...
    if (addCount > 0) {
      const alreadyAllocatedItemIds = new Set<string>();
      for (const ea of currentAllocs) {
        alreadyAllocatedItemIds.add(ea.inventoryItemId);
      }
      for (const a of allocations) {
        alreadyAllocatedItemIds.add(a.inventoryItemId);
      }

/**
 * URL publique canonique de l'application (Core / Worker / Web).
 *
 * En production, HTTPS et un hostname public sont obligatoires.
...
/**
 * Résout l'origine publique de façon sûre sans fallback arbitraire.
 */
export function getPublicAppUrl(environment: NodeJS.ProcessEnv = process.env): string {
  const rawValue = environment.PUBLIC_APP_URL || environment.NEXT_PUBLIC_APP_URL;

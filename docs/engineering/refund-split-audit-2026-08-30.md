...
Les corrections suivantes restent bloquées par des décisions humaines et ne
doivent pas être devinées dans le code :

1. base de remboursement : total client, base marketplace ou autre base ;
2. sort de chacun des composants 13 % et 7 % lors d'un remboursement total ou
   partiel ;
3. frais Stripe non récupérables, reverse transfer, chargebacks et soldes
   négatifs ;
4. traitement d'une annulation après amendement financier ou après plusieurs
   paiements ;
5. délai annoncé au client, notifications et escalade d'un échec provider.
...
3. Décider le comportement d'annulation après amendement, puis adapter le
   calcul et le cap multi-origines (`REFUND-AUDIT-02`).
4. Après sign-off Finance/Juridique, versionner la politique et implémenter le

...
puis [ADR-029](../decisions/ADR-029-marketplace-fee-split-13-7.md) et, pour les
remboursements ou diminutions après amendement, [ADR-030](../decisions/ADR-030-split-refund-policy.md).
La règle technique `split-13-7-v1` est portée par le registre serveur de
...
- G7B-R3 (arbitrage produit et ADR-018) : cycle courant. Décisions produit
  ...
  réservation) : Accepted — conception approuvée, implémentation terminée (G7P-A, G7P-B1, G7P-B2-A/B/C).
...
- G7M-C1 implémenté : `createSupplementBookingAmendment` crée localement et
  ...
  définitive a depuis été validée par CI post-merge (15 jobs parallèles, verts). Voir
  ...
  12/12 et `handle-webhook` 93/93, 0 skip). Les corrections de revue ont été
  intégrées ; la validation Core globale définitive a depuis été validée par CI post-merge (15 jobs parallèles, verts). Voir
  ...
  entièrement implémenté et validé par 26/26 tests Core C4-B (`supplement-compensation.integration.test.ts`),
...
- G7I (validation transversale Lot 7) : implémenté et validé par la CI de la
  ...
  fichier source modifié. Fusion sur main effectuée et validée (commit `19653fac`, CI post-merge 15 jobs parallèles verts).

...
| 4 | client_secret non loggé dans les erreurs fermées | `packages/core/src/payment-initiation/initiate-payment.integration.test.ts` — vérifie `paymentJson.not.toContain('client_secret')`, `packages/core/src/webhook-handler/errors.ts` (doc: « Le client_secret et les données de carte ne doivent JAMAIS apparaître »), `packages/core/src/booking-drafts/errors.ts` (BookingDraftError) | `packages/core/src/integration/g7i-security-sentinel.test.ts` — static analysis sentinel fail-closed (découverte récursive des call sites WebhookHandlerError + BookingDraftError, aucune fuite de client_secret/SQL/provider, vérification des codes fermés) | Couvert | none |
| 5 | Navigation clavier, responsive mobile, erreurs accessibles (Web) | `apps/web/src/app/dashboard/[orgId]/page.g7g.test.ts` (pattern static analysis), `apps/web/src/app/[locale]/search/search-form.tsx` (htmlFor, aria-describedby), `apps/web/src/app/[locale]/search/search-results.tsx` (aria-live, role), `apps/web/src/app/checkout/[draftId]/checkout-client.tsx` (role="alert", aria-busy) | `apps/web/src/app/g7i-a11y-ux.test.ts` — static accessibility guardrails (labels, rôles, live regions, media queries — analyse statique du source) | Couvert | none |
...
| 8 | NEUTRAL amendments | `packages/core/src/booking-amendments/create-neutral-booking-amendment.integration.test.ts` | N/A (couverture existante) | Couvert | none |
| 9 | REFUND amendments | `packages/core/src/booking-amendments/create-refund-booking-amendment.integration.test.ts` | N/A (couverture existante) | Couvert | none |
| 10 | SUPPLEMENT amendments | `packages/core/src/booking-amendments/create-supplement-booking-amendment.integration.test.ts` | N/A (couverture existante) | Couvert | none |
| 11 | Initiation du supplément | `packages/core/src/booking-amendments/initiate-supplement-payment.integration.test.ts` | N/A (couverture existante) | Couvert | none |
| 12 | Webhook et application du supplément | `packages/core/src/booking-amendments/apply-supplement-amendment.integration.test.ts` | N/A (couverture existante) | Couvert | none |
| 13 | Expiration des amendements | `packages/core/src/booking-amendments/supplement-lifecycle.integration.test.ts` (lignes 403-453) | N/A (couverture existante) | Couvert | none |
| 14 | Retry des amendements | `packages/core/src/booking-amendments/supplement-lifecycle.integration.test.ts` (lignes 543-605) | N/A (couverture existante) | Couvert | none |
| 15 | Compensation des suppléments | `packages/core/src/booking-amendments/supplement-compensation.integration.test.ts` | N/A (couverture existante) | Couvert | none |
| 16 | Projection getEffectiveBooking | `packages/core/src/booking-amendments/get-effective-booking.integration.test.ts` | N/A (couverture existante) | Couvert | none |
| 17 | Analytics production fail-closed | `packages/core/src/product-analytics/g7h-b-analytics-wiring.integration.test.ts` | N/A (couverture existante) | Couvert | none |
| 18 | Outbox fermée et versionnée | `packages/core/src/transactional-documents/booking-confirmed-parser.test.ts` (BOOKING_CONFIRMED.v1 strict) | N/A (couverture existante) | Couvert | none |

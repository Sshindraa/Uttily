...
| Règles précises par catégorie (nombre minimum, vues requises, angles) | G8B-3B4 | Produit / technique | **Partiellement résolu pour le vélo** — le contrat et l’UI utilisent trois slots narratifs (`HERO_PROFILE`, `THREE_QUARTER_FRONT`, `SECONDARY_VIEW`) ; `product_photos.slot_type` est persisté depuis la migration `0040`. Le Photo Coach, les overlays, la checklist et la liste canonique partagée par les actions serveur sont livrés. Le gate de publication vérifie encore seulement trois checksums distincts ; l’enforcement par slot reste à finaliser. Les règles des autres catégories restent ouvertes. |

...
| Durée par défaut et bornes d'un bloc de maintenance au retour | Activation du workflow de maintenance opérationnel | Produit / opérations | Ouvert — AB applique provisoirement 24 h par défaut, avec choix comptoir 4 h / 24 h / 48 h / 7 j et bornes serveur de 15 min à 7 j ; confirmer la durée par catégorie, l'extension et le comportement à échéance. |
...
| Slug et activation de la famille paddle | Lot de préparation puis activation paddle | Produit / direction / technique | **Résolu le 2026-09-01** — `paddleboard` est le slug canonique `ACTIVE`, affiché « Paddle » en FR et « Stand-up paddle » en EN ; `paddle` reste historique et inchangé. Les seules options sont `single`/`tandem` et `rigid`/`inflatable` dans les attributs de variante existants, sans dimension obligatoire ni conversion historique. |
| Slug et activation du pédalo | Lot d'activation pédalo | Produit / direction / technique | **Résolu le 2026-09-01 par ADR-037** — `pedalboat` est le slug unique `ACTIVE`, affiché « Pédalo » / « Pedal boat », avec capacité facultative dans les attributs de variante existants. La migration idempotente 0055 expose les parcours génériques et le seuil universel de photos ; aucune conversion historique, règle spécialisée, autre catégorie ou moteur de compléments n'est ajouté. |
| Blocages manuels récurrents | Lot 21-U2-X — implémentation de la récurrence | Produit / technique / opérations | **Résolu le 2026-09-02 par ADR-038 Accepted** — une série porte sur un seul exemplaire, hebdomadaire, bornée à 12 semaines, dans le fuseau IANA explicite de l'établissement ; les occurrences sont des `MANUAL_BLOCK` matérialisés. Les horaires inexistants ou ambigus DST et tout conflit sont refusés ; les occurrences passées ou commencées restent inchangées, la suspension ne libère rien et toute libération est explicite. Aucune exception, série infinie, multi-exemplaires, cron ou worker en V1. |

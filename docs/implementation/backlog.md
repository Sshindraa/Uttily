...
| G7E — Routes/pages/carte/i18n & Pont Checkout | page/route, UX/a11y, carte déplaçable/zoomable, relance viewport, FR+EN, alternatives séparées — **G7E-A/B implémentés ; Pont Recherche Publique → Booking Checkout (G7E / Pont Checkout) implémenté et durci (migration 0038 avec public_id sur product_variants, read model getPublicOfferDetails réduit, résolveur resolvePublicBookingAuthority fail-closed, Server Action createBookingDraftAction avec hold atomique et idempotence déterministe, voir `docs/delivery/public-offer-booking-bridge-delivery.md`)** | G7D | dashboard, géocodeur configuré |

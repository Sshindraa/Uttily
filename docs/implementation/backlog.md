...
| G8B-3B — Onboarding accompagné vélo | collecte structurée, déroulé opérateur, checklist et audit du dashboard | G8B-3A | **En cours — kit, établissement, plans tarifaires et readiness livrés ; Photo Coach livré en partie ; badge professionnel et gate sémantique restant à cadrer/implémenter** |
...
| G8B-3B4 — Standard visuel et confiance vélo | trois vues guidées et validées côté serveur, identité professionnelle factuelle, conception du badge vérifié | G8B-3B2, ADR-020, ADR-026 | **Partiellement livré le 2026-08-27 : Photo Coach UI, overlays, checklist, contrat `BIKE_PHOTO_SLOTS`, migration `0040` et persistance `slot_type` sont en place. Reste à aligner l’allow-list serveur, à décider/implémenter le gate de publication par slots et à implémenter le badge professionnel ; aucune analyse IA n’est activée.** |

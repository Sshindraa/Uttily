...
| 21-U2-U — Mise à jour groupée de l’état physique des exemplaires | mutation Core transactionnelle et idempotente pour appliquer `NEW`, `GOOD`, `FAIR`, `POOR` ou `BROKEN` à plusieurs exemplaires ; verrouillage tenant-safe et validation complète avant écriture ; sélection, avertissement de non-réservabilité et résultat accessibles dans la flotte générique, sans modifier statuts, lieux, réservations, maintenances ni mouvements | **En cours — tranche Phase 2 ; aucune migration, validation PostgreSQL et Browser Clerk TEST déléguées à la CI** |

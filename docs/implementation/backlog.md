...
| G8B-3B4 — Standard visuel et confiance vélo | trois vues guidées et validées côté serveur, identité professionnelle factuelle, conception du badge vérifié | G8B-3B2, ADR-020, ADR-026 | **Partiellement livré le 2026-08-27 : Photo Coach UI, overlays, checklist, contrat `BIKE_PHOTO_SLOTS`, migration `0040`, persistance `slot_type` et allow-list canonique partagée sont en place. Reste à décider/implémenter le gate de publication par slots et à implémenter le badge professionnel ; aucune analyse IA n’est activée.** |

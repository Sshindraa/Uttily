...
| 21-U2-Q — Duplication contrôlée d’un équipement | action générique loueur pour les huit familles `ACTIVE` ; nouveau produit `DRAFT` à slug unique, copie du catalogue et des variantes descriptives uniquement, idempotence et verrou d’organisation, checklist de publication reprise ; aucun stock, tarif, snapshot, photo, réservation, maintenance ou publication copié | **Livré le 2026-09-02 — commits `1ae7189` et `52cd976`, PR #58, CI complète verte avec Browser Clerk TEST ; intégration PostgreSQL ciblée validée par la CI, aucune migration** |
| 21-U2-R — Création en série d’exemplaires | mutation Core atomique et rejouable pour les huit familles `ACTIVE` ; variante, établissement et tenant vérifiés avant écriture ; limite 50, SKU déterministes et uniques, conflit explicite ; onboarding et surfaces flotte génériques réutilisent la mutation sans toucher aux réservations, maintenances ni disponibilités | **Livré le 2026-09-02 — commit `4af4103`, PR #59, CI complète verte avec Browser Clerk TEST ; intégration PostgreSQL ciblée validée par la CI, aucune migration** |
| 21-U2-S — Transfert groupé d’exemplaires entre établissements | mutation Core transactionnelle et atomique pour plusieurs exemplaires d’une même organisation ; verrouillage déterministe, un mouvement par exemplaire, idempotence de l’opération, conflit de clé et no-op explicites ; sélection multiple et confirmation accessibles dans la flotte générique, sans toucher aux statuts, conditions, réservations ni disponibilités | **Livré le 2026-09-02 — commit `6fdd00a`, PR #60, CI complète verte avec Browser Clerk TEST ; tests PostgreSQL ciblés exécutés par la CI, aucune migration** |
| 21-U2-T — Changement groupé de statut des exemplaires | mutation Core transactionnelle et idempotente pour passer plusieurs exemplaires à `ACTIVE`, `RETIRED` ou `LOST` ; verrouillage tenant-safe et validation complète avant écriture ; sélection, confirmation et résultat accessibles dans la flotte générique, sans modifier condition, mouvements, réservations, maintenances ou disponibilités | **Livré le 2026-09-02 — tests ciblés Core/Web et typechecks verts ; validation PostgreSQL complète et Browser Clerk TEST déléguées à la CI, aucune migration** |
| 21-U2-U — Mise à jour groupée de l’état physique des exemplaires | mutation Core transactionnelle et idempotente pour appliquer `NEW`, `GOOD`, `FAIR`, `POOR` ou `BROKEN` à plusieurs exemplaires ; verrouillage tenant-safe et validation complète avant écriture ; sélection, avertissement de non-réservabilité et résultat accessibles dans la flotte générique, sans modifier statuts, lieux, réservations, maintenances ni mouvements | **Livré le 2026-09-02 — PR #62 fusionnée en squash dans `450f896`, CI complète et Browser Clerk TEST verts ; aucune migration** |
| 21-U2-V — Blocage manuel ponctuel d’un exemplaire | mutation Core transactionnelle et idempotente sur `MANUAL_BLOCK`, conversion du fuseau de l’établissement vers UTC, contrainte anti-chevauchement existante, libération `RELEASED`, action flotte générique et impact disponibilité sans changement d’état, de condition, de réservation ou de maintenance | **Implémenté le 2026-09-02 — tests ciblés Core/Web, typechecks verts ; intégration PostgreSQL ciblée et Browser Clerk TEST délégués à la CI** |
| 21-U2-W — Visibilité des blocages manuels dans le planning | read model planning avec événements `MANUAL_BLOCK` actifs filtrés par établissement/exemplaire, bornés à la fenêtre et accompagnés du fuseau IANA ; affichage « Indisponible » et état « Bloqué » dans les vues planning/flotte, compteurs séparés et libellés issus du registre | **Implémenté le 2026-09-02 — tests ciblés Core/Web et typechecks verts ; intégration PostgreSQL et Browser Clerk TEST déléguées à la CI** |
| 21-U2-X — Blocages manuels récurrents | séries hebdomadaires finies sur un exemplaire, 12 semaines maximum, dates/heures murales dans le fuseau IANA de l'établissement, occurrences `MANUAL_BLOCK` matérialisées atomiquement, conflits fail-closed, idempotence, verrouillage, cycle de vie et action flotte/planning générique ; aucune récurrence infinie, exception ou modification silencieuse des occurrences commencées | **Implémenté le 2026-09-02 — ADR-038 Accepted, migration 0056, tests Core/Web ciblés et typechecks ; validation PostgreSQL complète et Browser Clerk TEST déléguées à la CI** |
| 21-U2-Y — Calendrier détaillé par exemplaire | read model serveur mono-exemplaire fondé sur les blocs opérationnels existants (`HOLD`, réservation, maintenance, blocage manuel ponctuel/récurrent), filtres tenant/exemplaire/établissement, fenêtre UTC bornée et affichage IANA, événements tronqués, liens vers les détails existants, timeline flotte/planning responsive et accessible ; aucune mutation, migration, nouvelle règle de disponibilité ou export | **Implémenté le 2026-09-02 — tests ciblés Core/Web, typechecks et non-régressions ; intégration PostgreSQL et Browser Clerk TEST à confirmer par la CI** |
| 21-U2-Z — Cockpit opérationnel quotidien & handovers comptoir | read model fulfillment tenant-safe avec lieu, date civile locale et recherche serveur UUID/référence/SKU ; buckets exclusifs `PICKUPS_TODAY`, `OVERDUE`, `RETURNS_TODAY`, `ONGOING` ; `DeskView` URL-driven, flows départ/retour en tiroir accessible, revalidation serveur et idempotence réutilisée ; aucun champ financier, PII client supplémentaire, import ou migration | **Implémenté le 2026-09-02 — tests ciblés Core/Web, typechecks, lint et format vérifiés ; intégration PostgreSQL et Browser Clerk TEST déléguées à la CI** |
| 21-U2-AA — Incidents de comptoir : No-Show et Substitution d’exemplaire au départ | no-show avec libération transactionnelle et traçabilité sans modification financière ; substitution atomique d’un exemplaire par un équivalent disponible du même établissement ; mutations serveur idempotentes et UI comptoir accessible dans `DeskView` | **Implémentation locale terminée le 2026-09-03 — tests unitaires Core, intégration PostgreSQL ajoutée, typechecks/lint/format et `check:fast` verts ; PostgreSQL non exécuté localement faute de `DATABASE_URL`** |
| 21-U2-AB — Incidents de retour : Dommages bloquants et mise en maintenance automatique | restitution et constat de dommage transactionnels ; condition `BROKEN`/maintenance immédiate protégée par un bloc `MAINTENANCE` borné (défaut 24 h, durée ajustable au comptoir) ; conflit avec une réservation ferme tracé par `CONFLICT_DETECTED` sans modification financière ; durée et impact visibles dans `ReturnFlow` | **Implémentation locale terminée le 2026-09-03 — tests unitaires Core, intégration PostgreSQL ajoutée, `check:fast`, lint et format verts ; PostgreSQL non exécuté localement faute de `DATABASE_URL`, aucune migration** |
| 21-U2-AC — Traitement des retards critiques : Déclaration de non-restitution et clôture de dossier | réservation `ACTIVE` strictement échue (`now() > customerEndAt`) ; mutation transactionnelle et idempotente ; tous les exemplaires alloués passent à `LOST`, les blocs résiduels passent à `RELEASED` et le dossier passe à `CLOSED` ; audit/outbox `BOOKING_DECLARED_LOST` avec circonstances ; aucun recalcul ni changement des montants, taxes ou snapshots financiers | **Implémentation locale terminée le 2026-09-03 — tests unitaires Core, intégration PostgreSQL ajoutée, `check:fast`, lint et format verts ; PostgreSQL non exécuté localement faute de `DATABASE_URL`, aucune migration** |
| 21-U2-AD — Réservation manuelle au comptoir (Walk-in & Téléphone) | création de réservation immédiate ou par téléphone pour un client présent ou appelant ; calcul automatique de tarif et devis d'après les plans tarifaires (flexible-pricing-v1/legacy) ; recherche en direct du matériel disponible sur le créneau (anti-surbooking) ; bloc `BOOKING`/`ACTIVE` et allocation instantanée sous contrainte GiST ; règlements comptoir audités (TPE/Carte, Espèces, Chèques-Vacances, Chèque, Paiement au retour) ; bouton direct dans `DeskView` et vue ergonomique `CounterBookingView` | **Implémenté et testé sur PostgreSQL le 2026-09-03 — 5 tests d'intégration PostgreSQL verts, Server Actions et page Next.js `/dashboard/[orgId]/bookings/new`, `check:fast`, lint, format et build validés** |
| 21-L1 — Surfaces légales publiques et textes contractuels canoniques | publication des pages légales publiques accessibles et localisées (`/terms`, `/rental-terms`, `/privacy`, `/legal`) en version `v1` canonique ; intégration des liens contractuels dans le footer client et notice de consentement opposable dans le checkout ; routes publiques Clerk autorisées et redirections d'alias racine (`/cgu`, `/cgv`, etc.) | **Implémentation locale terminée le 2026-09-03 — tests unitaires Web, `check:fast`, lint et format verts ; pages v1 prêtes pour validation juridique humaine** |
| 21-L2 — Contrat Loueur & Conditions Pro B2B (Conditions Générales Partenaires) | publication de la page contractuelle publique bilingue `/pro-terms` (version `v1` canonique) détaillant le modèle de split 13/7, la conformité de flotte, la substitution obligatoire, l'obligation d'assurance RC Pro et le mandat de reversement Stripe Connect ; case à cocher d'acceptation obligatoire dans l'onboarding organisation avec enregistrement immuable dans `audit_log` (`ORGANIZATION_PRO_TERMS_ACCEPTED`) | **Implémentation locale terminée le 2026-09-03 — tests unitaires Core/Web, typechecks, lint/format et `check:fast` verts ; contrat v1 prêt pour validation juridique humaine** |
| 21-O1 — Informations légales et fiscales du loueur partenaire | identification formelle du loueur partenaire (migration 0057) : colonnes SIRET/SIREN avec validation Luhn, numéro de TVA intracommunautaire (calcul et format), forme juridique (`LEGAL_FORMS`), RCS, ville d'immatriculation, capital social, siège social et représentant légal ; écran de gestion Pro `/dashboard/[orgId]/settings` avec journalisation immuable (`ORGANIZATION_LEGAL_SETTINGS_UPDATED`) ; intégration dans le read model de readiness loueur | **Livré et fusionné sur `main` (PR #78) le 2026-09-04 — tests unitaires, validation Luhn/TVA, 8 tests PostgreSQL, `check:fast` et CI complète verts** |
| 21-F1 — Reçus financiers déterministes, mentions légales et décompte de commission | pipeline documentaire financier complet : reçu de location acquitté (`RENTAL_RECEIPT`) et décompte de commission/reversement loueur (`COMMISSION_STATEMENT`) générés déterministement au format PDF avec mentions légales obligatoires complètes (Uttily SAS intermédiaire, loueur vendeur, TVA/art. 293 B) ; routes d'accès client (`/account`) et loueur (`/dashboard/[orgId]/finances/statement`) | **Livré et fusionné sur `main` (PR #78) le 2026-09-04 — tests unitaires Core/Web/Worker, génération PDF vérifiée, `check:fast` et CI complète verts** |
| 21-P1 — Fondation RGPD client & registre des demandes de droits | table canonique `privacy_requests` (migration 0058), formulaire client bilingue `/[locale]/account/privacy`, calcul d'échéance légale (+1 mois calendaire), journalisation append-only sans PII ; endpoint d'export sécurisé `/api/account/privacy/export` scindé en copie intégrale Art. 15 (`buildPersonalDataCopy`, sans secrets ni IDs Stripe) et export portable Art. 20 (`buildPortableData`) | **Livré et fusionné sur `main` (PR #78) le 2026-09-04 — tests unitaires Core/Web, tests d'intégration PostgreSQL réels, `check:fast` et CI complète verts** |
| 21-P1A — Cockpit régalien support RGPD | interface interne d'instruction `/internal/privacy` avec filtrage par statut et type de droit ; Server Actions sécurisées pour prise en charge (`TAKE_IN_CHARGE`), décision régalienne (`DECIDE`) avec motifs fermés (acceptation, refus légal, prolongation 2 mois justifiée), attestation d'envoi et clôture effective ; outbox events et audit trail complets | **Livré et fusionné sur `main` (PR #78) le 2026-09-04 — tests unitaires Core/Web, typecheck 9/9, `check:fast` et CI complète verts** |
| 21-S1 — Simulation opérationnelle locale d'un samedi type | scénario d'intégration bout-en-bout (`scripts/saturday-drill.mjs`) simulant une journée complète d'exploitation 08h00 - 20h30 sur PostgreSQL réel : création de compte avec mentions légales (21-O1), réservation walk-in (21-U2-AD), substitution au comptoir (21-U2-AA), restitution avec dommage (21-U2-AB), décompte financier (21-F1) et export RGPD (21-P1) | **Livré et fusionné sur `main` (PR #78) le 2026-09-04 — exécutable via `pnpm drill:saturday`, testé dans la CI et en local** |
| ADR-030 — Politique de remboursement split par delta | calcul composant par composant (`HALF_UP_PER_COMPONENT`) du delta entre états effectifs successifs d'une réservation ; exécution des remboursements intégraux via Stripe ; protection fail-closed des remboursements partiels en attente de validation humaine | **Livré et fusionné sur `main` (PR #78) le 2026-09-04 — tests d'intégration PostgreSQL, runtime vérifié, CI complète verte** |
| 21-P1C — Registre des sous-traitants RGPD & Décision DPO-005 | registre d'instruction et fiches de conformité pour les 6 sous-traitants (Clerk, Stripe, Neon, Cloudflare, Resend, Vercel) avec bases de transfert UE/USA (DPF et SCC 2021/914) ; dossier prêt pour signature formelle DPO | **Livré et fusionné sur `main` (PR #78) le 2026-09-04 — document [`21-p1c-subprocessors-dpo-005.md`](../operations/21-p1c-subprocessors-dpo-005.md) validé** |
| 21-P2 — Effacement RGPD du compte utilisateur & scellement probatoire (PRIVACY-ERASURE) | implémentation conforme aux arbitrages souverains DPO-003 & DPO-004 (ADR-039) : table `privacy_probatory_seals` (migration 0059), workflow de suppression autonome en libre-service (`eraseMyAccountAction`) avec modale de confirmation et section Zone de Danger dans `/[locale]/account/privacy`, workflow support dans le cockpit `/internal/privacy` (`executeSupportErasureAction`) ; pseudonymisation irréversible de `users` (`email` -> tombstone, `displayName` -> null, `oidcSubject` -> tombstone, `deletedAt = NOW()`), purge immédiate du compte Clerk, scellement probatoire (5 ans responsabilité civile Art. 2224 Code civil / 10 ans comptabilité Art. L. 123-22 Code de commerce), préservation de l'intégrité référentielle `audit_log`, garde-fous fail-closed (réservations actives/en cours, propriétaire unique d'inventaire) et blocage de réauthentification (`AccountDeletedError`) | **Implémenté et validé le 2026-09-04 — migration 0059, ADR-039, 12 tests unitaires Core, 16 tests Web, tests d'intégration PostgreSQL, typecheck 9/9 et `pnpm check:fast` verts (<90s)** |

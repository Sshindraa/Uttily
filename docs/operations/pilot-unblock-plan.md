...
| `C2A-06` | Base de calcul des refunds | `LEGAL-005`, `FIN-002` | Juridique + finance | `READY_FOR_HUMAN_DECISION` | Le code calcule sur `booking.totalAmountMinor` (option A / total TTC) ; une proposition delta entre états effectifs, composant par composant, est formalisée dans `ADR-030`. | Valider et versionner la base proposée, puis comparer explicitement au comportement actuel. | Oui |

...
describe.skipIf(shouldSkip)('createBookingDraftAction — intégration PostgreSQL & concurrence', () => {
  ...
  async function seedFixture(
    ...
    await sql`
      ...
      VALUES ('FR', true, 'EUR', 'fr-FR')
      ON CONFLICT ("country_code") DO UPDATE SET "is_active" = true
    `;

    // Org
    const org = await sql`
      INSERT INTO "organizations" ("legal_name", "public_display_name", "slug")
      VALUES (${'Org ' + tag}, ${'Loueur ' + tag}, ${'org-' + tag})
      RETURNING "id"
    `.then((r) => r[0]!);
  ...
  it('1. Happy path : Crée un booking draft avec hold atomique et retourne redirectUrl', async () => {
    ...
    expect(Number(draftRows[0]!.total_amount_minor)).toBe(10000); // 2 jours x 5000
  });

  it('2. Concurrence réelle : 2 requêtes simultanées sur le dernier exemplaire -> exactement 1 hold créé', async () => {
    ...
    // Exécuter 2 créations concurrentes pour le même créneau
    const [res1, res2] = await Promise.all([
      (async () => {
        vi.mocked(auth.getAuthenticatedUser).mockResolvedValue(user1);
        return createBookingDraftAction({
          publicProductId: f.publicProductId,
          publicLocationId: f.publicLocationId,
          variantId: f.variantId,
          quantity: 1,
      ...
      (async () => {
        vi.mocked(auth.getAuthenticatedUser).mockResolvedValue(user2);
        return createBookingDraftAction({
          publicProductId: f.publicProductId,
          publicLocationId: f.publicLocationId,
          variantId: f.variantId,
  ...
  it('3. Idempotence : un second appel avec la même idempotencyKey rejoue le résultat sans hold supplémentaire', async () => {
    ...
    // Vérifier qu'un seul draft a été créé
